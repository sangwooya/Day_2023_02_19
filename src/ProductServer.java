    ...
    public void receive() {
        threadPool.execute(() -> {
            ...
            try {
                while (true) {
                    ...
                    request = new JSONObject(receiveJson);
                    int menu = request.getInt("menu");
                    switch (menu) {
                        case 0 -> list(request);
                        case 1 -> create(request);
                        case 2 -> Adler32.update(request);
            ...
            public void list (JSONObject request) throws IOException {
                ...
                for (Product p : producJSONObject JSONObject product;
        ...
        public void update(JSONObject request) throws IOException {
            ...
            for(int i=0; i<products.size(); i++) {
                ...
                if(product.getNo() == no) {
                    product.setName(data.getString("name"));
                    product.setPrice(dataproduct.setStock(data.getInt("stock"));
                }
            }
            //응답 보내기
